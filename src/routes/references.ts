...
router.put("/sedes/:id", authMiddleware, async (req: AuthRequest, res) => {
  try {
    if (req.user?.roleId !== 1) return res.status(403).json({ error: "Solo admins" });
    const sede = await prisma.sede.update({ where: { id: parseId(req.params.id) }, data: req.body });
    res.json(sede);
  } catch (error) {
...
router.delete("/sedes/:id", authMiddleware, async (req: AuthRequest, res) => {
  try {
    if (req.user?.roleId !== 1) return res.status(403).json({ error: "Solo admins" });
    await prisma.sede.update({ where: { id: parseId(req.params.id) }, data: { isActive: false } });
...
router.put("/unidades/:id", authMiddleware, async (req: AuthRequest, res) => {
  try {
    if (req.user?.roleId !== 1) return res.status(403).json({ error: "Solo admins" });
    const unidad = await prisma.unidadNegocio.update({ where: { id: parseId(req.params.id) }, data: req.body });
    res.json(unidad);
  } catch (error) {
...
router.delete("/unidades/:id", authMiddleware, async (req: AuthRequest, res) => {
  try {
    if (req.user?.roleId !== 1) return res.status(403).json({ error: "Solo admins" });
    await prisma.unidadNegocio.update({ where: { id: parseId(req.params.id) }, data: { isActive: false } });
...
router.put("/cargos/:id", authMiddleware, async (req: AuthRequest, res) => {
  try {
    if (req.user?.roleId !== 1) return res.status(403).json({ error: "Solo admins" });
    const cargo = await prisma.cargo.update({ where: { id: parseId(req.params.id) }, data: req.body });
    res.json(cargo);
  } catch (error) {
...
router.delete("/cargos/:id", authMiddleware, async (req: AuthRequest, res) => {
  try {
    if (req.user?.roleId !== 1) return res.status(403).json({ error: "Solo admins" });
    await prisma.cargo.update({ where: { id: parseId(req.params.id) }, data: { isActive: false } });
